...
class RankingBeatsWordCountTests(unittest.TestCase):
    ...
    def test_short_relevant_document_outranks_long_noise(self) -> None:
        top = self.index.search("parse config", top_k=3)
        self.assertEqual(top[0].doc_id, "short_answer")

    def test_raw_word_count_would_have_ranked_the_noise_first(self) -> None:
    ...
    def test_unrelated_document_is_not_returned(self) -> None:
        returned = {r.doc_id for r in self.index.search("parse config")}
        self.assertNotIn("other", returned)


class SaturationTests(unittest.TestCase):
    def test_score_gain_shrinks_as_frequency_grows(self) -> None:
    ...
    def test_score_is_bounded_by_k1_plus_one_times_idf(self) -> None:
        index = BM25Index()
...
class IdfTests(unittest.TestCase):
    ...
    def test_unknown_term_scores_zero(self) -> None:
        index = BM25Index()
...
class SearchContractTests(unittest.TestCase):
    ...
    def test_add_many_matches_repeated_add(self) -> None:
        one = BM25Index()
    ...
    def test_score_rejects_a_bad_index(self) -> None:
...
class ExplainTests(unittest.TestCase):
    ...
    def test_unknown_document_returns_nothing(self) -> None:
        self.assertEqual(BM25Index().explain("q", "missing"), {})


class FusionTests(unittest.TestCase):
    def test_a_document_ranked_well_by_both_wins(self) -> None:
        fused = reciprocal_rank_fusion([["a", "b", "c"], ["a", "c", "b"]])
        self.assertEqual(fused[0][0], "a")

    def test_documents_from_either_list_are_included(self) -> None:
        ids = {doc for doc, _ in
               reciprocal_rank_fusion([["a", "b"], ["c", "d"]])}
        self.assertEqual(ids, {"a", "b", "c", "d"})

    def test_fusion_is_deterministic_on_ties(self) -> None:
    ...
    def test_hybrid_search_blends_both_rankers(self) -> None:
        index = BM25Index()
...
class RealCorpusTests(unittest.TestCase):
    ...
    def test_query_finds_the_right_module(self) -> None:
        import io
        import os
        index = BM25Index()
