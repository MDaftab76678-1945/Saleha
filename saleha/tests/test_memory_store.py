...
class MemoryStoreTests(unittest.TestCase):
    ...
    def test_remember_and_persistence(self) -> None:
    ...
    def test_exact_and_fuzzy_recall(self) -> None:
        self.store.remember(
    ...
    def test_search_and_tag_filtering(self) -> None:
        self.store.remember(
    ...
    def test_export_and_import_json(self) -> None:
        self.store.remember("Task Export 1", "Code Export 1", tags=["export"])
        export_file = os.path.join(self.tmpdir.name, "exported.json")
        self.assertTrue(self.store.export_json(export_file))
        self.assertTrue(os.path.isfile(export_file))

        # Import into fresh store
        fresh_file = os.path.join(self.tmpdir.name, "fresh_store.json")
        fresh_store = MemoryStore(storage_path=fresh_file)
        self.assertEqual(len(fresh_store.list_all()), 0)

        imported_count = fresh_store.import_json(export_file)
        self.assertEqual(imported_count, 1)
        self.assertEqual(len(fresh_store.list_all()), 1)
        self.assertEqual(fresh_store.list_all()[0].goal, "Task Export 1")

    def test_cli_memory_endpoints(self) -> None:
        # Test stats
    ...
    def test_compact_conversation_history(self) -> None:
        steps = [
...
class ModelScopedRecallTests(unittest.TestCase):
    ...
    def test_unfiltered_recall_still_shares_across_models(self) -> None:
    ...
    def test_recall_scoped_to_the_producing_model_hits(self) -> None:
        hit = self.store.recall("write a fibonacci function",
                                model="qwen2.5-coder:3b")
        self.assertIsNotNone(hit)
        self.assertEqual(hit.model, "qwen2.5-coder:3b")

    def test_recall_scoped_to_a_different_model_misses(self) -> None:
    ...
    def test_fuzzy_match_is_also_model_scoped(self) -> None:
    ...
    def test_empty_store_scoped_recall_is_safe(self) -> None:
    ...
    def test_orchestrator_does_not_filter_on_literal_auto(self) -> None:
