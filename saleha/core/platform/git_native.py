...
class GitAutomationEngine:
    ...
    def create_task_branch(self, goal: str) -> str:
        """Generates a clean semantic branch name, checks it out, and returns
        the branch name only if the checkout actually succeeded.

        The checkout's return code used to be ignored, so this returned a
        truthy branch name string even when `git checkout` failed (e.g. a
        locked index) -- a caller checking `if created:` to decide
        `branch_created = True` (repo_orchestrator.py) had no way to tell a
        real branch switch from a no-op. Reproduced directly: with the git
        index locked, checkout failed, the current branch never changed,
        and this still returned the branch name.
        """
        if not self.is_git_repo():
            return ""
