...
def load_all_datasets(dataset_paths: list[str]):
    all_samples = []
...
def main():
    ...
    # 4-Bit NormalFloat Quantization Config (100% Native on GPU)
    ...
    # High-Capacity Rank-32 LoRA Config for Advanced Mathematical Convergence
    ...
    dataset_files = [
        "datasets/saleha_asi_math_reasoning_train.json",
    ...
    table.add_row("LoRA Capacity", "Rank-32 / Alpha-64 (59.8M Trainable Weights)")
    table.add_row("Quantization Scheme", "4-Bit NF4 (Double Quantization)")
    table.add_row("GPU Hardware", f"{gpu_name} (6.0 GB VRAM)")
    table.add_row("Total Samples Fed", f"{len(raw_dataset)} ASI Mathematics Samples")
