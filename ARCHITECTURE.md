...
- **`cognitive`** (`cognitive_engine.py`) — **four text heuristics; the dangerous claim removed 2026-09-07.** It scored a "Temporal/Spatial/Ethical/Reasoning Vector", each one regex, with `ast` imported and never called (the `mech_interp.py` defect again). The real problem was the **positive claim**: a regex matching nothing was reported as an assurance. Measured — code POSTing a user's private keys to a remote host scored **100/EXCELLENT** with "Zero unconsented telemetry or surveillance mechanisms found", while the literal string `x = "telemetry"` in a comment scored 75. Every "nothing found" message now states what was actually searched, and the summary says the score is four text patterns with arbitrary weights, not a measurement.
- **`constitutional-check`** (`constitutional_guard.py`) — **a four-pattern regex screen; the compliance verdict removed 2026-09-07.** Its docstring claimed five rules including "No unauthorized network sockets" and "No obfuscated payload execution"; there were four rules and neither of those had a pattern. `is_compliant` was True whenever no regex matched, so code that walks `/` deleting every file, ships `/etc/passwd` to a remote socket and execs a downloaded payload was reported **COMPLIANT**. `is_compliant` is gone, replaced by `matched_rules`; the summary names the four patterns checked, says a miss is not a safety verdict, and points at `saleha sast` for the real AST scanner. It still has nothing to do with Constitutional AI as the term is used — no model, no self-critique, no revision.
...
To build the desktop app locally, `pip install -e ".[desktop]"` (or `.[dev]`, which also includes it) is required first -- `pnpm build`/`pnpm dev` in `apps/desktop` shells out to `scripts/build_desktop_sidecar.py`, which needs PyInstaller to bundle the Python backend into the sidecar binary Tauri launches. Without it, the build fails immediately with "No module named PyInstaller"; this was undocumented and untested until it was found and fixed here.
