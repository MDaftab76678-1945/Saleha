...
- **`explain-code`** (`mech_interp.py`) — **template.** "Mechanistic interpretability" here means four `substring in line` keyword buckets with a fixed score per bucket (e.g. any of `raise`/`try:`/`except` → score 0.95, "defensive error guard circuit"). No model weights, activations, or attention are ever inspected — the module imports `ast` and `re` and never calls either.
- **`emergence-check`** (CLI wiring only; `emergence_detector.py` itself is real, see above) — **template in this wiring.** The CLI calls the real detector's `evaluate_swarm_health()` with zero data ever fed to it (no code path in the repo calls this singleton's `record_message()`), so the command deterministically prints "Swarm communication is idle and healthy" every time, regardless of actual agent activity.
...
- **`recursive`** (`recursive_solver.py`) — **real, but misnamed.** Real LLM calls (via `BaseAgent.think()`) and real sandboxed self-healing execution genuinely happen. But the advertised "7-node network" is one class making a linear sequence of ~5 method calls (not recursive, not a network), and its centerpiece "multi-path reasoning" step returns 3 hardcoded `ReasoningPath` objects with fixed scores that never read the actual goal — the "winning trajectory" is deterministically always the same one for any input.
