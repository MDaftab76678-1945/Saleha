...
## [Unreleased] — audit passes 1-136 (2026-09-02 onward)

Since the 2.6.0 entry below, development has been a continuous
correctness audit rather than a feature release, so no version has been
cut. The work is tracked pass by pass in
[NOTEBOOK_IMPORT.md](NOTEBOOK_IMPORT.md), with a running summary in
`CLAUDE.md`. It is not duplicated here; this section exists so the
changelog does not silently imply nothing has happened since 2026-09-02.

Shape of the work, for orientation:

- **Removing fabricated results.** Commands and engines that reported
  success, scores, or security verdicts they had not computed were found
  and either made real or deleted. Examples: the SWE-bench exporter that
  graded the model against its own answer key (pass 51), a sidecar daemon
  whose "Auto-Fix" returned the caller's own unmodified code (pass 133),
  a team pipeline that read a failed security review as `APPROVED`
  (pass 135).
- **Making the agent loop honest.** Roughly thirty passes (53, 85-111)
  on `agentic_loop.py`, adding gates so a repair cannot be reported as
  done unless a real test run, a revert check, and a coverage check all
  agree.
- **Verification counts.** Test suite 783 (at 2.6.0) → **2337 passed,
  13 skipped, 172 subtests**, 0 failures. CLI commands: **162
  registered**.

### Corrections to the 2.6.0 entry below

- **`saleha leaderboard` no longer exists.** It was deleted in pass 30,
  not fixed: it printed a comparison of Saleha against Devin, Claude Code
  and Cursor from hand-typed numbers, with no real measurement behind any
  of them. There is no honest version of that command until real
  cross-tool measurement exists, so it belongs in `ROADMAP.md` rather
...
### Hardware Profiler (deep telemetry)
...
### Aider-level Context Engine (optional `[codeintel]` extra)
...
### B: Plugin Hooks Wired (real for the first time)
...
### Real Multimodal Vision (last genuine stub closed)
...
### Memory & Vector Store Performance
...
### Container Preflight & Sandboxing
...
### Bug Fixes & Network Race Resolution
...
### Deployment & State Machine Fixes
...
### Execution Policy & Container Sandboxing
...
### Model Catalog & SmartRouter Intelligence
...
### Dynamic Import & Swarm Security Gates
...
### API Authentication & Sandbox Isolation
