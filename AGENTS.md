# AGENTS.md — Universal Agent Operating System & Engineering Contract

> **Project:** saleha-0.1 (Local-First Autonomous Multi-Agent AI Coding Assistant)  
> **Audience:** Claude Code, Gemini / Antigravity IDE, Cursor, Saleha Core Agents

This document is the absolute, authoritative operating contract for any AI agent working in this repository.
It codifies the user's vision, physical runtime boundaries, past defect graveyards, and engineering principles.
Every agent must load and abide by these rules before reading or touching any code.

---

## 1. The North Star: The User's Vision for Saleha

Saleha is not just another wrapper; it is an autonomous, self-improving coding intelligence designed under strict local constraints:
...
1. **NO COMMANDS WITHOUT EXPLICIT PERMISSION:**
   - Never execute any shell command (`run_command`) without clearly telling the user what it does and obtaining approval.
...
- **Python Virtual Environments:**
  - `.\.venv\` (Python 3.14.7): **Default development environment.** All daily coding, auditing, and testing MUST run here.
  - `.\.venv_train\` (Python 3.11.16): Reserved strictly for offline torch/PEFT training scripts. Never run everyday tests here — it is below the `requires-python = ">=3.12"` floor and CI never tests it.
- **Local Ollama Networking & Constraints:**
  - Installed models (verified via `ollama list`, 2026-09-20): `qwen2.5-coder:3b` (1.9 GB, primary fast coder), `qwen3:8b` (5.2 GB), `qwen3.5:9b` (6.6 GB), `qwen3.5:4b` (3.4 GB), `deepseek-coder:6.7b` (3.8 GB), `deepseek-r1:7b` (4.7 GB), `nomic-embed-text` (274 MB, embeddings), `gemma4:31b-cloud`.
