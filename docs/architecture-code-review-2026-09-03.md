...
| 4 | `saleha/core/platform/self_healer.py` | 119–126 | `run_command()` runs an arbitrary `command` string via `subprocess.run(..., shell=True)`. Lower risk than #1 since it's driven by internal test/build commands rather than a raw HTTP field, but confirm nothing upstream ever builds `command` from agent-generated or file-derived text before it reaches here. | Security |
